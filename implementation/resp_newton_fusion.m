function [disp_row, disp_col, sind, max_scale_response, response,w] = resp_newton_fusion(response1, response2, iterations, ky, kx, use_sz)
...
if numel(response2) ~= 1 && numel(response1) ~= 1
    ...
    for j = 1:numel(weight)
        ...
        ep = min(r_r(:));
        L(j) = 0.4*(weight(j)^2+(1-weight(j))^2)-ep;
    end
    [~, jidx]=min(L(:));
