...
for i = 1:num_feature_blocks
    sz = filter_sz_cell{i};
    tmp_sz0 = base_target_sz/feature_cell_sz(i);
    tmp_sz0(tmp_sz0 < 1) = 1;
    output_sigma = sqrt(prod(floor(tmp_sz0))) * params.output_sigma_factor;
...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            [dy_pred, dx_pred] = exponential_smoothing(dy_true, dy_pred, dx_true, dx_pred, params.alpha);
            % get the estimated position
            center = pos + [dy_pred dx_pred];
            est_pos = round(center);
            sample_scale = currentScaleFactor*scaleFactors;
            
            % extract features
            xt = extract_features(im, est_pos, sample_scale, features, global_fparams, feature_extract_info);
            ...
            scores_fs_handcrafted = scores_fs_feat{k1}; 
            for k = block_inds
                scores_fs_feat{k} = gather(sum(bsxfun(@times, conj(cf_f{k}), xtf{k}), 3));
                scores_fs_feat{k} = resizeDFT2(scores_fs_feat{k}, output_sz);
                scores_fs_deep = scores_fs_feat{k};
            end
            
            % feature fusion
            scores_fs_handcrafted2 = permute(gather(scores_fs_handcrafted), [1 2 4 3]); 
            scores_fs_deep2 = permute(gather(scores_fs_deep), [1 2 4 3]); 
            responsef_padded_handcrafted = resizeDFT2(scores_fs_handcrafted2, output_sz);
            response_handcrafted = ifft2(responsef_padded_handcrafted, 'symmetric');
            responsef_padded_deep = resizeDFT2(scores_fs_deep2, output_sz);
            response_deep = ifft2(responsef_padded_deep, 'symmetric');
            [disp_row, disp_col, sind, ~, response, ~] = resp_newton_fusion(squeeze(response_handcrafted)/2, squeeze(response_deep)/1,...
            ...
            old_pos = pos;
            pos = est_pos + translation_vec;
            
            % failure detection and correction
            quality = resp_quality(response);            
    ...
    tracking_result.target_size = double(init_target_sz);
