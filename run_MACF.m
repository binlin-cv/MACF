function results = run_MACF(seq, res_path, bSaveImage)
...
cnn_params.output_layer = [8];           % Which layers to use
...
params.clamp_position = 1;          % Clamp the target position to be inside the image

% Learning parameters
params.output_sigma_factor = 1/13;		% Label function sigma
params.temporal_regularization_factor = [10 10]; % The temporal regularization parameters
...
params.number_of_scales = 1;            % Number of scales to run the detector (without scale estimation)
params.scale_step = 1.01;               % The scale factor

% Smoothing constant
params.alpha = 0.2;

% Falure correction parameters
params.resp_budg = [];
params.resp_norm = 0;
